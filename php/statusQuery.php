...
if (isset($_SESSION['stallname'])) {
    ...
    // SQL query to count orders with status "ACTIVE" for today and for the logged-in user's stallname
    $sql = "SELECT COUNT(*) AS total_orders FROM orders WHERE status = 'ACTIVE' AND DATE(order_date) =? AND stallname =?";
